        ...
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
            ...
            <form action="{{ route('pesan.store') }}" method="POST" class="bg-white shadow rounded p-6"
                ...
                <div class="mb-4">
                    ...
                    <select id="jenis" name="jenis" required class="w-full border rounded px-3 py-2"
                        x-model="jenis">
                        <option value="" disabled>Pilih Jenis</option>
                        <option value="camilan" {{ old('jenis') == 'camilan' ? 'selected' : '' }}>Camilan</option>
                        <option value="non-camilan" {{ old('jenis') == 'non-camilan' ? 'selected' : '' }}>Non-Camilan
                        </option>
                    </select>
                </div>
                ...
                <div class="mb-4">
                    ...
                    <select id="status" name="status" required class="w-full border rounded px-3 py-2"
                        ...
                        <option value="pengiriman" {{ old('status') == 'pengiriman' ? 'selected' : '' }}>Pengiriman
                        </option>
                ...
                <!-- Harga per kg atau per barang -->
                <x-input-number label="Harga per Barang (Rp)" name="harga" required x-model.number="harga"
                    step="1000" />

                <!-- Berat untuk camilan -->
                <template x-if="jenis === 'camilan'">
                    <x-input-number label="Berat (kg)" name="berat" step="0.01" required x-model.number="berat" />
                </template>

                <!-- Jumlah untuk non-camilan -->
                ...
                <div class="mb-6 font-semibold text-lg text-blue-600">
                    Total Jastip:
                    <span
                        x-text="totalJastip.toLocaleString('id-ID', { style: 'currency', currency: 'IDR' })">Rp0</span>
                </div>

                <div class="mb-6 font-semibold text-lg text-green-700 border-t pt-3">
                    Total Keseluruhan:
                    <span
                        x-text="(totalHarga + totalJastip).toLocaleString('id-ID', { style: 'currency', currency: 'IDR' })">Rp0</span>
                </div>
